...
from atlaswiki.errors import AppError

VERSION = "0.1.0"

app = FastAPI(title="AtlasWiki", version=VERSION, docs_url="/api/docs", openapi_url="/api/openapi.json")
...
async def health() -> dict:
    from atlaswiki.jobs import job_queue  # 延迟导入避免循环
...
# --- 路由注册（必须在静态托管之前） ---
from atlaswiki.api import ask, events, lint, pages, settings, sources, system  # noqa: E402
from atlaswiki.api import compile as _compile  # noqa: E402
...
def run() -> None:
    """CLI 入口：uv run python -m atlaswiki"""
    import socket
    ...
    for p in range(8765, 8776):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            if s.connect_ex(("127.0.0.1", p)) != 0:
                port = p
                break
    print(f"AtlasWiki v{VERSION} → http://127.0.0.1:{port}")
